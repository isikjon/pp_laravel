...
class ImportSpbGirls extends Command
{
    protected $signature = 'girls:import-spb {--fresh : Truncate SPB girls before import}';
    protected $description = 'Import SPB girls data from JSON files in parser_xlsx/anketas_spb';
    ...
    public function handle()
    {
        ...
        if ($this->option('fresh')) {
            $this->info('Deleting SPB girls from table...');
            Girl::where('city', 'Санкт-Петербург')->delete();
        }
        ...
        $this->info("Total SPB in DB: " . Girl::where('city', 'Санкт-Петербург')->count());
    ...
    private function parseJsonData($data)
    {
        ...
        return [
            ...
            'city' => $data['contact_info']['city'] ?? 'Санкт-Петербург',
            'metro' => $metro,
            'district' => $district,
