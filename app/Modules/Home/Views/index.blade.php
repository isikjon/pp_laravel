...
@section('page_scripts')
    ...
    <script>
        ...
        const filtersInteractiveSelectors = ['.filtersBtn', '.more-info', '.btn-formFilterModal__btn'];
        ...
        function loadFiltersScript(callback) {
            ...
            whenJqueryReady(function () {
                ...
                script.addEventListener('error', function () {
                    filtersScriptLoading = false;
                    const pending = filtersScriptCallbacks.splice(0, filtersScriptCallbacks.length);
                    pending.forEach(function (fn) {
                        try { fn(new Error('failed to load filters.js')); } catch (callbackError) { console.error('[filters-loader] callback error', callbackError); }
                    });
        ...
        function cloneMouseEventSnapshot(event) {
            return {
                type: event.type,
        ...
        function redispatchClick(target, snapshot) {
            if (!target) {
                return;
            }
            ...
            if (typeof window.MouseEvent === 'function') {
                clonedEvent = new MouseEvent(snapshot.type, options);
            } else {
                clonedEvent = document.createEvent('MouseEvent');
        ...
        function shouldBootstrapInteraction(event) {
            for (let i = 0; i < filtersInteractiveSelectors.length; i++) {
                if (event.target.closest(filtersInteractiveSelectors[i])) {
                    return true;
                }
            }
            return false;
        ...
        function getInteractiveTarget(event) {
            for (let i = 0; i < filtersInteractiveSelectors.length; i++) {
                const target = event.target.closest(filtersInteractiveSelectors[i]);
                if (target) {
                    return target;
                }
            }
        ...
        function handleDeferredInteractiveClick(event) {
            if (!shouldBootstrapInteraction(event)) {
                return;
            }
            ...
            if (filtersScriptLoaded) {
                document.removeEventListener('click', handleDeferredInteractiveClick, true);
                return;
            }

            const snapshot = cloneMouseEventSnapshot(event);
            ...
            loadFiltersScript(function (error) {
                if (error) {
                    return;
                }
                ...
                requestAnimationFrame(function () {
                    redispatchClick(target, snapshot);
                });
            });
        }
        ...
        document.addEventListener('click', handleDeferredInteractiveClick, true);
    </script>
@endsection
