...
@section('title', 'Стрип-клубы в ' . (($cityName ?? 'Москва') === 'Санкт-Петербург' ? 'Санкт-Петербурге' : 'Москве'))
    ...
    <section class="mainContent">
        <div class="container">
            <div class="textSection">
                <h1>
                    Стрип-клубы в {{ ($cityName ?? 'Москва') === 'Санкт-Петербург' ? 'Санкт-Петербурге' : 'Москве' }}
                </h1>
                <p>
                    Лучшие стрип-клубы {{ $cityName === 'Санкт-Петербург' ? 'Санкт-Петербурга' : 'Москвы' }} – профессиональные танцовщицы и незабываемые шоу. Посетите наши заведения и насладитесь атмосферой роскоши и страсти.
                </p>
            </div>
            ...
            <div class="clubsSection" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(350px, 1fr)); gap: 30px; margin-top: 40px;">
                @foreach($clubs as $club)
                    <div class="clubCard" style="background: #fff; border-radius: 15px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.1); transition: transform 0.3s;">
                        @if(!empty($club->images) && is_array($club->images))
                            ...
                            <div style="height: 250px; overflow: hidden; position: relative;">
                                <a href="{{ $fullImage }}" class="club-lightbox-trigger" data-club-id="{{ $club->club_id }}" onclick="event.stopPropagation();">
                                    <img src="{{ $firstImage }}" alt="{{ $club->name }}" class="club-preview-img" style="width: 100%; height: 100%; object-fit: cover;">
                                </a>
                            </div>
                        @else
                            <div style="height: 250px; overflow: hidden; position: relative;">
                        ...
                        <a href="{{ route('stripclub.show', $club->club_id) }}" style="text-decoration: none; color: inherit;">
                            <div style="padding: 20px;">
                                ...
                                <div style="margin-bottom: 10px; display: flex; align-items: center; gap: 8px;">
                                    ...
                                    <span>{{ $club->metro ?? 'Метро не указано' }}</span>
                                </div>
                                
                                @if(!empty($club->phones))
                                    <div style="margin-bottom: 10px; display: flex; align-items: center; gap: 8px;">
                                        ...
                                        <span>{{ $club->phones[0] }}</span>
                                    </div>
                                @endif
                                
                                <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #eee;">
                                    <span style="color: #7E1D32; font-weight: 600;">{{ $club->schedule ?? 'Круглосуточно' }}</span>
                                </div>
                            </div>
                        </a>
                    </div>
            ...
            <div class="paginationGirls" style="margin-top: 40px;">
                ...
                @else
                    <a href="{{ $clubs->previousPageUrl() }}" class="arrowPagination arrowPagination-prev">
                        <img src="{{ asset('img/arrowLeft.svg') }}" alt="">
                    </a>
                @endif
                ...
                <div class="pagination__paginationGirls">
                    @for($i = 1; $i <= $clubs->lastPage(); $i++)
                        <a href="{{ $clubs->url($i) }}" class="block-paginationGirls {{ $i == $clubs->currentPage() ? 'block-paginationGirls__active' : '' }}">
                            {{ $i }}
                        </a>
                    @endfor
                </div>
                ...
                @if($clubs->hasMorePages())
                    <a href="{{ $clubs->nextPageUrl() }}" class="arrowPagination arrowPagination-next">
    ...
    if (typeof lightGallery !== 'undefined') {
        document.querySelectorAll('.club-lightbox-trigger').forEach(function(trigger) {
            trigger.addEventListener('click', function(e) {
                ...
                const clubName = this.querySelector('img').getAttribute('alt');
                
                const tempContainer = document.createElement('div');
                tempContainer.style.display = 'none';
                tempContainer.innerHTML = '<a href="' + imgUrl + '"><img src="' + imgUrl + '" alt="' + clubName + '"></a>';
